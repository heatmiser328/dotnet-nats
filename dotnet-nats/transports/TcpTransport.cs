...
namespace dotnet_nats
{
    public class TcpTransport : ITransport
    {
        ...
        public TcpTransport(string address, int port)
        {
            ...
            _args.SocketFlags = SocketFlags.None;
            _args.SetBuffer(new byte[cBufferSize], 0, cBufferSize);
        }
        ...
        public async Task<bool> Open(string address = null, int port = -1)
        {
            try
            {
                ...
                _args.RemoteEndPoint = server;
                var awaitable = new SocketAwaitable(_args);
        ...
        async Task<bool> Receive()
        {
            try
            {
                ...
                {
                    message.AddRange(_args.Buffer.Take(bytes).ToList());
                }
